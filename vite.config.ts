...
function sfinderRoutePlugin() {
  const installRewrite = (server: { middlewares: { use: (handler: (request: { url?: string }, response: unknown, next: () => void) => void) => void } }) => {
    server.middlewares.use((request, _response, next) => {
      ...
      if (pathname === "/dictionary") request.url = "/dictionary.html";
      next();
    });
  };
  return {
...
export default defineConfig({
  ...
  optimizeDeps: {
    entries: ["index.html", "replay.html", "licence.html", "solver.html", "sfinder.html", "setup_test.html", "dictionary.html"],
  },
  build: {
    rollupOptions: {
      input: {
