...
let promise;
export async function loadBatchWasm(){promise??=(async()=>{const wasm=await bytesFor(new URL('../wasm/batch_wasm.wasm',import.meta.url));const{instance}=await WebAssembly.instantiate(wasm,{});return instance.exports})();return promise}
...
export class BatchReachability{
 ...
 tSpinKind(board,cells){return Number(this.e.batch_tspin_kind(board,cells,this.height,this.physics))}
 #run({base=0n,operations,queues=[],mode='normal',useHold=true}){
  ...
  const count=Number(this.e.batch_engine_run(base,this.height,this.physics,modeCode,useHold?1:0));if(count===0xffffffff)throw new Error('batch engine failed');
  const variants=[];
  for(let vi=0;vi<count;vi++){
   const ids=this.e.batch_engine_variant_ids(vi),clears=Number(this.e.batch_engine_variant_clears(vi)),tspins=Number(this.e.batch_engine_variant_tspins(vi)),pcMask=Number(this.e.batch_engine_variant_pc_mask(vi));
 ...
 congruent({base=0n,fill=0n,queues=[],useHold=true,maxSolutions=20000}){
  ...
  const count=Number(this.e.batch_congruent_run(base,fill,this.height,this.physics,useHold?1:0,maxSolutions));if(count===0xffffffff)throw new Error(`congruent tiling limit ${maxSolutions} reached`);
  const out=[];for(let si=0;si<count;si++){const n=Number(this.e.batch_congruent_operation_count(si)),operations=[];for(let oi=0;oi<n;oi++){const p=Number(this.e.batch_congruent_operation_piece(si,oi));operations.push({piece:CODE_PIECE[p],mask:this.e.batch_congruent_operation_mask(si,oi)})}const orders=[];for(let i=0,qn=Number(this.e.batch_congruent_order_count(si));i<qn;i++){const packed=this.e.batch_congruent_order(si,i);let order='';for(let j=0;j<n;j++)order+=CODE_PIECE[Number((packed>>BigInt(j*3))&7n)];orders.push(order)}out.push({operations,orders})}return out;
 }
}
