...
import {fieldMasks,aggregateMasks} from './batch-geometry.mjs';
...
function decodedTargets(sourceFumen,height){return decodedCoverTargets(decoder.decode(sourceFumen),height)}
...
export async function calculateCover({sourceFumen,pattern,clear=4,mode='normal',mirror='no',useHold=true}){
 ...
 const finderPattern=queuesForFinder(pattern),queues=expandPatternCases(finderPattern),targets=decodedTargets(sourceFumen,clear);
 const heights=[...new Set(targets.map(target=>target._batchHeight??clear))];
 const reachabilityByHeight=new Map();
 for(const height of heights)reachabilityByHeight.set(height,await batchReachability(height,'jstris'));
 const reachability=reachabilityByHeight.get(clear)??reachabilityByHeight.values().next().value;
 const result=coverTargets({targets,queues,height:clear,reachability,reachabilityForHeight:height=>reachabilityByHeight.get(height),useHold,mirror:boolMirror(mirror),mode});
...
export async function calculateCongruent({sourceFumen,pattern,clear=4,blueGarbage=false,useHold=true,_keepVariants=false}){
 validateTargetLines(clear);const pages=decoder.decode(sourceFumen);if(!pages.length)throw new Error('input Fumen has no pages');
